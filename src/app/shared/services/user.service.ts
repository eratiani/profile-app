...
export class UserService {
  ...
  addUserData(userDTO: IUser) {
    return this.http.post<IUser>(`${this.API}/userData`, userDTO).pipe(
      catchError((err) => {
  ...
  getUser(id: string): Observable<IUser> {
    return this.http.get<IUser>(`${this.API}/userData/${id}`).pipe(
      catchError((err) => {
        this.messageService.clear();
        const errorMessage =
          err.error?.message || err.message || 'An unknown error occurred';
  ...
  deleteUser(id: string) {
    return this.http.delete(`${this.API}/userData/${id}`).pipe(
      catchError((err) => {
        this.messageService.clear();
        const errorMessage =
          err.error?.message || err.message || 'An unknown error occurred';
  ...
  getUserData(): Observable<IUser[] | []> {
    return this.http.get<IUser[]>(`${this.API}/userData`).pipe(
      catchError((err) => {
        this.messageService.clear();
        const errorMessage =
          err.error?.message || err.message || 'An unknown error occurred';
